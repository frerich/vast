...
// A custom stream manager that is able to notify when all data has been processed.
// It relies on the presence of a function `Self->state.notify_flush_listeners()`,
// which means that it is currently only usable in combination with the `index` actor.
...
// This is essentially a copy of `caf::attach_continous_stream_stage()`, but since
// the construction of the `stream_stage_impl` is buried quite deep there it is
// necesssary to duplicate the code.
...
attach_notifying_stream_stage(Self* self, bool continuous, Init init, Fun fun,
                               Finalize fin = {},
  ...
  static_assert(std::is_same<
                  void(state_type&, caf::downstream<output_type>&, input_type),
  ...
  auto ptr = caf::make_counted<impl>(self, std::move(init), std::move(fun), std::move(fin));
  if (continuous)
    ptr->continuous(true);
  return ptr;
}
